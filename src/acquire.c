...
void acquire_process(acquire_t *st)
{
    ...
    {
    	if (st->fir_q15_execute_fm_run != NULL && st->mode == NRSC5_MODE_FM) {
    		unsigned int length = FFTCP_FM * (ACQUIRE_SYMBOLS + 1);
    		unsigned int input_bytes_len = length * sizeof(cint16_t);
    		unsigned int output_bytes_len = length * sizeof(complex float);

    		if (xrtBOWrite(st->fir_q15_execute_fm_input, st->in_buffer, input_bytes_len, 0)) {
    			printf("fir_q15_execute_fm_run: error transferring data from host to dev\n");
    		}
    		if (xrtBOSync(st->fir_q15_execute_fm_input, XCL_BO_SYNC_BO_TO_DEVICE, input_bytes_len, 0)) {
    			printf("fir_q15_execute_fm_run: error syncing data from host to dev\n");
    		}
    		if (xrtRunStart(st->fir_q15_execute_fm_run)) {
    			printf("fir_q15_execute_fm_run: error starting kernel\n");
    		}
    		if (xrtRunWait(st->fir_q15_execute_fm_run) == ERT_CMD_STATE_ABORT) {
    			printf("fir_q15_execute_fm_run: error run aborted\n");
    		}
    		if (xrtBOSync(st->fir_q15_execute_fm_output, XCL_BO_SYNC_BO_FROM_DEVICE, output_bytes_len, 0)) {
    			printf("fir_q15_execute_fm_run: error syncing buffer from device to host\n");
    		}
    		if (xrtBORead(st->fir_q15_execute_fm_output, st->buffer, output_bytes_len, 0)) {
    			printf("fir_q15_execute_fm_run: error transferring data from device to host\n");
    		}
    	} else {
			cint16_t y;
...
void acquire_init(acquire_t *st, input_t *input)
{
    ...
    st->fir_q15_execute_fm_run = input->fir_q15_execute_fm_run;
    st->fir_q15_execute_fm_input = input->fir_q15_execute_fm_input;
    st->fir_q15_execute_fm_output = input->fir_q15_execute_fm_output;
