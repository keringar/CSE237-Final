...
void input_push_cu8(input_t *st, uint8_t *buf, uint32_t len)
{
	...
	if (st->radio->fir_q15_execute_decim_run != NULL) {
		if (xrtBOWrite(st->radio->fir_q15_execute_decim_input, buf, len, 0)) {
			printf("error transferring data from host to dev\n");
		}
		if (xrtBOSync(st->radio->fir_q15_execute_decim_input, XCL_BO_SYNC_BO_TO_DEVICE, len, 0)) {
			printf("error syncing data from host to dev\n");
		}
		if (xrtRunSetArg(st->radio->fir_q15_execute_decim_run, 2, len)) {
			printf("error setting arg 3\n");
		}
		if (xrtRunStart(st->radio->fir_q15_execute_decim_run)) {
			printf("error starting kernel\n");
		}
		if (xrtRunWait(st->radio->fir_q15_execute_decim_run) == ERT_CMD_STATE_ABORT) {
			printf("error run aborted\n");
		}
		if (xrtBOSync(st->radio->fir_q15_execute_decim_output, XCL_BO_SYNC_BO_FROM_DEVICE, len, 0)) {
			printf("error syncing buffer from device to host\n");
		}
		if (xrtBORead(st->radio->fir_q15_execute_decim_output, &st->buffer[st->avail], len, 0)) {
		...
		for (i = 0; i < len; i += 4)
		{
			...
			{
//				printf("kernel: %d %d", st->buffer[st->avail].r, st->buffer[st->avail].i);
				halfband_q15_execute(st->decim[0], x, &st->buffer[st->avail++]);
//				printf("halfband: %d %d", st->buffer[st->avail - 1].r, st->buffer[st->avail- 1].i);
			}
			else
			{
				x[0].r >>= 4;
...
void input_init(input_t *st, nrsc5_t *radio, output_t *output)
{
    ...
    st->fir_q15_execute_fm_run = radio->fir_q15_execute_fm_run;
    st->fir_q15_execute_fm_input = radio->fir_q15_execute_fm_input;
    st->fir_q15_execute_fm_output = radio->fir_q15_execute_fm_output;
