...
static int parse_args(state_t *st, int argc, char *argv[])
{
    ...
    while ((opt = getopt_long(argc, argv, "r:w:o:d:p:g:ql:vH:TD:x:", long_opts, NULL)) != -1)
    {
        switch (opt)
        {
        case 1:
...
int main(int argc, char *argv[])
{
    ...
    if (st->input_name)
    {
        ...
        if (nrsc5_open_file(&radio, fp, st->xcl_bin) != 0)
        {
    ...
    else if (st->rtltcp_host)
    {
        ...
        if (nrsc5_open_rtltcp(&radio, s, st->xcl_bin) != 0)
        {
    ...
    {
        if (nrsc5_open(&radio, st->device_index, st->xcl_bin) != 0)
        {
