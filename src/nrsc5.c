...
static void nrsc5_init(nrsc5_t *st, const char* xcl_bin)
{
	if (xcl_bin != NULL) {
		printf("loading %s\n", xcl_bin);
		st->device_handle = xrtDeviceOpen(0);
		st->xclbin = xrtXclbinAllocFilename(xcl_bin);
		if (xrtDeviceLoadXclbinHandle(st->device_handle, st->xclbin)) {
			printf("Could not load XCL bin");
		}

		xuid_t device_uuid;
		xrtDeviceGetXclbinUUID(st->device_handle, device_uuid);

		st->fir_q15_execute_decim_kernel = xrtPLKernelOpen(st->device_handle, device_uuid, "fir_q15_execute_decim");
		xrtMemoryGroup bank_grp_idx_0 = xrtKernelArgGroupId(st->fir_q15_execute_decim_kernel, 0);
		st->fir_q15_execute_decim_input = xrtBOAlloc(st->device_handle, 524288, XRT_BO_FLAGS_NONE, bank_grp_idx_0);
		xrtMemoryGroup bank_grp_idx_1 = xrtKernelArgGroupId(st->fir_q15_execute_decim_kernel, 1);
		st->fir_q15_execute_decim_output = xrtBOAlloc(st->device_handle, 524288, XRT_BO_FLAGS_NONE, bank_grp_idx_1);
		if (st->fir_q15_execute_decim_input == NULL || st->fir_q15_execute_decim_output == NULL) {
			printf("failed to allocate XBO's\n");
		}

		st->fir_q15_execute_decim_run = xrtRunOpen(st->fir_q15_execute_decim_kernel);
		xrtRunSetArg(st->fir_q15_execute_decim_run, 0, st->fir_q15_execute_decim_input);
		xrtRunSetArg(st->fir_q15_execute_decim_run, 1, st->fir_q15_execute_decim_output);

		xrtDeviceGetXclbinUUID(st->device_handle, device_uuid);

		st->fir_q15_execute_fm_kernel = xrtPLKernelOpenExclusive(st->device_handle, device_uuid, "fir_q15_execute_fm");
		xrtMemoryGroup fm_bank_grp_index_0 = xrtKernelArgGroupId(st->fir_q15_execute_fm_kernel, 0);
		st->fir_q15_execute_fm_input = xrtBOAlloc(st->device_handle, sizeof(cint16_t) * 140400, XRT_BO_FLAGS_NONE, fm_bank_grp_index_0);
		xrtMemoryGroup fm_bank_grp_index_1 = xrtKernelArgGroupId(st->fir_q15_execute_fm_kernel, 1);
		st->fir_q15_execute_fm_output = xrtBOAlloc(st->device_handle, sizeof(complex float) * 140400, XRT_BO_FLAGS_NONE, fm_bank_grp_index_1);
		st->fir_q15_execute_fm_run = xrtRunOpen(st->fir_q15_execute_fm_kernel);
		xrtRunSetArg(st->fir_q15_execute_fm_run, 0, st->fir_q15_execute_fm_input);
		xrtRunSetArg(st->fir_q15_execute_fm_run, 1, st->fir_q15_execute_fm_output);

		printf("done loading all kernels\n");
	}
...
NRSC5_API int nrsc5_open(nrsc5_t **result, int device_index, const char* xcl_bin)
{
    int err;
...
NRSC5_API int nrsc5_open_file(nrsc5_t **result, FILE *fp, const char* xcl_bin)
{
    nrsc5_t *st = nrsc5_alloc();
    st->iq_file = fp;
    nrsc5_init(st, xcl_bin);
...
NRSC5_API int nrsc5_open_pipe(nrsc5_t **result, const char* xcl_bin)
{
    nrsc5_t *st = nrsc5_alloc();
    nrsc5_init(st, xcl_bin);
...
NRSC5_API int nrsc5_open_rtltcp(nrsc5_t **result, int socket, const char* xcl_bin)
{
    int err;
    nrsc5_t *st = nrsc5_alloc();
...
NRSC5_API void nrsc5_stop(nrsc5_t *st)
{
    ...
    if (st->xclbin != NULL) {
		xrtRunClose(st->fir_q15_execute_decim_run);
		xrtBOFree(st->fir_q15_execute_decim_input);
		xrtBOFree(st->fir_q15_execute_decim_output);
		xrtKernelClose(st->fir_q15_execute_decim_kernel);

		xrtRunClose(st->fir_q15_execute_fm_run);
		xrtBOFree(st->fir_q15_execute_fm_input);
		xrtBOFree(st->fir_q15_execute_fm_output);
		xrtKernelClose(st->fir_q15_execute_fm_kernel);

		xrtDeviceClose(st->device_handle);
		xrtXclbinFreeHandle(st->xclbin);
    }
}
